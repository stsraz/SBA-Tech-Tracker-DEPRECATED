<?php
	class Accordion extends Presenter {
		...
		public $accordion_string;
		
		public function __construct($id,$location,$num_divs,$class_array,$hthree_array,$div_handle_array,$table_array) {
			$this->set_table_array($table_array);
			...
			$this->accordion_string=$this->build_accordion();
		}
		public function get_accordion_string() {
			return $this->accordion_string;
		}
		...
		public function set_table_array($table_array) {
		...
		public function build_accordion() {
			...
			for($i=0;$i<$this->num_divs;$i++) {
				$accordion_construct=$accordion_construct.'<h3>'.$this->hthree_array[$i].'</h3><div class="'.$this->class_array[$i].'" id="'.$this->div_handle_array[$i].'">'.$this->table_array[$i].'</div>';
			}
